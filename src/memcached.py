...
class MemcachedLocust(User):
    ...
    def __init__(self, MemcachedLocust):
        super().__init__(MemcachedLocust)
        ...
        self.client = LocustMemcachedClient((self.host, 11211), number_of_keys=self.number_of_keys)
...
class LocustMemcachedClient(Client):
    ...
    def __init__(self, conn, number_of_keys):
        ...
        for key in range(number_of_keys):
            m = hashlib.md5()
            m.update("{}".format(key).encode('utf-8'))
    ...
    def random_set(self):
        ...
        value = ''.join(random.choice(string.ascii_lowercase) for x in range(length))
        key = next(self._computed_keys)
        print(key)
